...
make_common <- function(df, pos, split_cols = NULL) {
  if (is.null(split_cols)) {
    ...
    ## of creating impossible long digits.  So we round to 4 digits before
    ## making the common size statement
    df <- dplyr::mutate_if(df, is.numeric, ~ round(., 4))
    ...
    common <- dplyr::mutate_if(common, is.numeric, ~ round(., 4))
    df <- cbind(df, common)
